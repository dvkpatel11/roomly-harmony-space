...
const AnimatedLogo: React.FC<AnimatedLogoProps> = ({ className, size = "md", showText = true }) => {
  const sizes = {
    sm: "w-6 h-6",
    md: "w-8 h-8",
    lg: "w-12 h-12",
  ...
  return (
    <div className={cn("flex items-center gap-2", className)}>
      <div
        className={cn(
          "relative aspect-square rounded-lg bg-primary text-primary-foreground flex items-center justify-center animate-logo-pulse",
          sizes[size]
        )}
      >
